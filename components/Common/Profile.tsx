...
export default function Profile(props:any){
    const data = props.data?props.data:{user:null};
    const [user, setUser] = React.useState({...data});
    const [errors, setErrors] = React.useState({name:null, email:null, whatsapp:null});
    ...
    const handleSubmit = (e:React.MouseEvent<HTMLButtonElement, MouseEvent>) =>{
        ...
        setFetching(true);
        axios
        .post("http://localhost:3000/api/auth/updateUserProfile", user)
        .then((res) => {
          toast.success("Updated Successfully.");
          setErrors({name:null, email:null, whatsapp:null});
        })
        .catch((err) => {
          if (err.response) {
